...
export default ({ strapi }) => ({
  async transfer(params: TransferParams): Promise<TransferResult> {
    const { senderClerkId, recipientClerkId, amount, senderName, recipientName } = params;
    ...
    return knex.transaction(async (trx) => {
      ...
      if (senderCards.length === 0) {
        return { success: false, error: 'Unlock your card to transfer' };
      }
      ...
      if (recipientCards.length === 0) {
        return { success: false, error: 'Recipient has no active card to receive funds' };
      }

      const recipientCard = recipientCards[0];

      // Calculate new balances
      const senderNewBalance = (Math.round(senderCard.balance * 100) - amountCents) / 100;
      const recipientNewBalance = (Math.round(recipientCard.balance * 100) + amountCents) / 100;
      ...
      const senderTxPublishedId = (await trx('transactions').max('id as id').first()).id;
      ...
      const recipientTxDocId = crypto.randomUUID().replace(/-/g, '').slice(0, 24);
      ...
      const recipientTxPublishedId = (await trx('transactions').max('id as id').first()).id;
